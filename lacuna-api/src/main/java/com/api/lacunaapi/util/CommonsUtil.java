...
//	}
    ...
    public static String getSubstringCredNet(String valor, int posicaoInicial, int tamanho) {
        //no crednet tem q retirar 1 na posicao inicial
        posicaoInicial = posicaoInicial -1;
        return valor.substring(posicaoInicial, posicaoInicial + tamanho ).trim();
    }


    public static Date stringDataHoraCredNet(String data, String hora) {

        if (data.equals("00000000"))
            return null;

        if (semValor(hora))
            return dateValue(data, "yyyyMMdd");
        else
            return dateValue(data + " " + hora, "yyyyMMdd HHmmss");
    }
}